...
If you want cleaner commit messages without AI-style generator trailers, install the local commit hook:

```bash
./scripts/install-git-hooks.sh
```

The hook removes lines such as `Made-with: ...`, `Generated by ...`, and AI-related `Co-authored-by` trailers from new commits.

To scrub existing commits on your current branch (before opening/merging a PR), you can run:

```bash
git filter-branch -f --msg-filter "sed -E '/^(Made-with:|Made with:|Generated by.*(AI|Cursor|ChatGPT|Claude|Copilot|GPT|LLM)|AI-generated:|Co-authored-by:.*(Cursor|ChatGPT|Claude|Copilot|GPT|LLM|AI))/Id'" HEAD
```

Then force-push the branch only if you are sure history rewrite is safe for your team workflow.
